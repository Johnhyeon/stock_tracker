...
def sync_mentions(
    ...
    텔레그램 채널에서 수집된 트레이더 관심종목 데이터를 DB에 저장합니다.
    """
    service = TraderService(db)
    try:
        result = service.sync_mentions(request.file_path)
        return SyncResponse(**result)
    except FileNotFoundError:
        raise HTTPException(status_code=404, detail="mentions.json 파일을 찾을 수 없습니다.")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"동기화 실패: {str(e)}")


@router.get("/hot", response_model=list[TraderHotStock])
def get_hot_stocks(
...
@router.get("/rising", response_model=list[TraderRisingStock])
...
@router.get("/performance", response_model=TraderPerformanceStats)
def get_performance_stats(
    ...
    트레이더들이 언급한 종목들의 평균 성과, 승률 등을 계산합니다.
    """
    service = TraderService(db)
...
def get_cross_check(
    ...
    내가 등록한 아이디어 종목 중 트레이더들도 주목하는 종목을 찾습니다.
    """
