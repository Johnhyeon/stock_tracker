...
class TraderMention(Base):
    """트레이더 종목 언급 기록.

    텔레그램 채널에서 수집된 트레이더들의 종목 언급 데이터.
    """
    __tablename__ = "trader_mentions"
    ...
    __table_args__ = (
        Index('ix_trader_mentions_stock_name', 'stock_name'),
        Index('ix_trader_mentions_stock_code', 'stock_code'),
        Index('ix_trader_mentions_date', 'mention_date'),
        Index('ix_trader_mentions_stock_date', 'stock_code', 'mention_date'),
    )

    def __repr__(self):
        return f"<TraderMention {self.stock_name} @ {self.mention_date}>"
...
class TraderStats(Base):
    ...
    __table_args__ = (
        Index('ix_trader_stats_stock_date', 'stock_code', 'stat_date', unique=True),
        Index('ix_trader_stats_date', 'stat_date'),
    )

    def __repr__(self):
        return f"<TraderStats {self.stock_name} @ {self.stat_date}>"
