...
from models import ExpertMention
...
class ThemeService:
    ...
    def analyze_theme_rotation(
        ...
        for stock in expert_data:
            ...
            for theme in themes:
                theme_scores[theme]["expert_mentions"] += mentions
                theme_scores[theme]["expert_stocks"].add(stock_code)
        ...
        for theme_name, data in theme_scores.items():
            all_stocks = data["youtube_stocks"] | data["expert_stocks"]
            ...
            mention_score = min(
                (data["youtube_mentions"] + data["expert_mentions"] * 2) / 50 * 40,
                40
            )
            ...
            theme_results.append({
                ...
                "expert_mentions": data["expert_mentions"],
    ...
    def get_theme_history(
        ...
        mentions = (
            self.db.query(
                ExpertMention.mentioned_date,
                func.count(ExpertMention.id).label("mention_count"),
                func.sum(ExpertMention.mention_count).label("total_mentions"),
            )
            .filter(
                ExpertMention.stock_code.in_(stock_codes),
                ExpertMention.mentioned_date >= start_date,
            )
            .group_by(ExpertMention.mentioned_date)
            .order_by(ExpertMention.mentioned_date)
            .all()
        )
